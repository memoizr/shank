...
inline fun <T> ShankModule.new(noinline factory: () -> T) = NewProvider<T>().also { factories.put(it.hashcode, factory) }
inline fun <A, T> ShankModule.new(noinline factory: (A) -> T) = NewProvider1<A, T>().also { factories.put(it.hashcode, factory) }
inline fun <A, B, T> ShankModule.new(noinline factory: (A, B) -> T) = NewProvider2<A, B, T>().also { factories.put(it.hashcode, factory) }
inline fun <A, B, C, T> ShankModule.new(noinline factory: (A, B, C) -> T) = NewProvider3<A, B, C, T>().also { factories.put(it.hashcode, factory) }
inline fun <A, B, C, D, T> ShankModule.new(noinline factory: (A, B, C, D) -> T) = NewProvider4<A, B, C, D, T>().also { factories.put(it.hashcode, factory) }
inline fun <A, B, C, D, E, T> ShankModule.new(noinline factory: (A, B, C, D, E) -> T) = NewProvider5<A, B, C, D, E, T>().also { factories.put(it.hashcode, factory) }
