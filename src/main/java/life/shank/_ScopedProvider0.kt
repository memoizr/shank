...
class _ScopedProvider1<A, T> : Provider<T, ScopedFactory.(A) -> T> {
    @Synchronized
    operator fun invoke(scope: Scope, a: A): T = get(scope, Params1(a)) { ii(SSF(scope), a) }
}

class _ScopedProvider2<A, B, T> : Provider<T, ScopedFactory.(A, B) -> T> {
    @Synchronized
    operator fun invoke(scope: Scope, a: A, b: B): T = get(scope, Params2(a, b)) { ii(SSF(scope), a, b) }
}

class _ScopedProvider3<A, B, C, T> : Provider<T, ScopedFactory.(A, B, C) -> T> {
    @Synchronized
...
interface ScopedProvider3<A, B, C, T> : Provider3<A, B, C, T> {
