...
abstract class ShankModule {
    ...
    fun <A, B, C, D, E, T> new(factory: (A, B, C, D, E) -> T) =
    ...
    fun <A, B, T> scoped(factory: ScopedFactory.(A, B) -> T) =
        ScopedProvider2(factory).also { factories.add(it to factory) }

    fun <A, B, C, T> scoped(factory: ScopedFactory.(A, B, C) -> T) =
        ScopedProvider3(factory).also { factories.add(it to factory) }

    fun <A, B, C, D, T> scoped(factory: ScopedFactory.(A, B, C, D) -> T) =
        ScopedProvider4(factory).also { factories.add(it to factory) }

    fun <A, B, C, D, E, T> scoped(factory: ScopedFactory.(A, B, C, D, E) -> T) =
